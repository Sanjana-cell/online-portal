...
{
    SqlConnection dbpath = new SqlConnection(ConfigurationManager.ConnectionStrings["OnlineMovieTicketConnectionString"].ConnectionString);
        ...
        try
        {
            if ((string)Session["PWD"] == fldoldpwd.Text)
            {
                if (fldnewpwd.Text == fldcnpwd.Text)
                {
                    ...
                    string str = "UPDATE [OnlineMovieTicket].[dbo].[ONM_login] SET lastupdate=CURRENT_TIMESTAMP ,password ='" + fldnewpwd.Text + "',uname='" + Session["UNAME"] + "' WHERE Eid ='" + Session["lid"] + "'";
